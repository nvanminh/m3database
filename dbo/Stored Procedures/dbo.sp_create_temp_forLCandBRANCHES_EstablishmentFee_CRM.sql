...
CREATE PROCEDURE [dbo].[sp_create_temp_forLCandBRANCHES_EstablishmentFee_CRM]
AS
BEGIN
	SET NOCOUNT ON;

	DELETE
	FROM [reporting_loan].[dbo].[temptbl_LCandBRANCHES_EstablishmentFee_CRM]

	INSERT INTO [reporting_loan].[dbo].[temptbl_LCandBRANCHES_EstablishmentFee_CRM]
	...
	WHERE iO_Product_Transaction.RTM_IDLink_RMR = iO_Product_MasterReference.RMR_ID
		...
		AND (
			...
			AND (iO_Control_TransactionMaster.XTRM_Detail = 'Money3\Fee\Loan\Establishment Fee')
			)
END
