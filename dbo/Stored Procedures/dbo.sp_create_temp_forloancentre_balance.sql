CREATE PROCEDURE [dbo].[sp_create_temp_forloancentre_balance]
AS
IF object_id('[reporting_loan].dbo.temptbl_westpac') IS NOT NULL
BEGIN
	TRUNCATE TABLE [reporting_loan].dbo.temptbl_westpac
END;
...
AS (
	...
	LEFT JOIN (
		...
		INNER JOIN iO_Control_Frequency
	...
	WHERE (
			...
			AND CMRBranchRef.CMR_Name = 'Loan Centre'
			AND BR.Cmr_Name IN ('Collections Branch')
			)
		AND cmr.cmr_seqnumber > 42671
		AND Products.RMR_SeqNumber IS NOT NULL
	)
INSERT INTO [reporting_loan].dbo.temptbl_westpac (
	[Branch],
	[BranchRefer],
	[ClientNo],
	[LoanNo],
...
SELECT c.[Branch],
	...
	c.[LodgeDate],
	c.[SettleDate],
	[FundingDate] = rcd.RCD_CurrentStart,
	c.[ReturnDate],
	c.[Credit Category],
	c.[Prod_Type],
	c.[Prod_Status],
	...
	c.[ReturnMonth]
FROM c
INNER JOIN iO_Product_MasterReference rmr
	ON c.LoanNo = rmr.RMR_SeqNumber
LEFT JOIN iO_Product_ControlDate rcd
	ON rmr.RMR_ID = rcd.RCD_IDLink_RMR
		AND RCD_Type = 5000
