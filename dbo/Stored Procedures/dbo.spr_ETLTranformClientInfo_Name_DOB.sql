...
CREATE PROC [dbo].[spr_ETLTranformClientInfo_Name_DOB]
AS
BEGIN
	...
	(
		...
		-- Bank Account Info
		'Unknown' AS BankAccountID, 'Unknown' AS BankAccount, 'Unknown' AS AccountName,
		-- Address Infor
		'Unknown'AS AddressID, 'Unknown' AS Street, 'Unknown' AS PostalCode, 'Unknown' AS OtherAddressInfo,
		-- Contact Infor
		'Unknown' AS Email, 'Unknown' AS Mobile, 'Unknown' AS OtherContactInfo,
		-- Timestamp
		...
		FROM iO_Client_MASterReference CM 
			LEFT JOIN iO_Client_TypeIndividual CT ON CM.CMR_ID = CT.CTI_IDLink_CMR
