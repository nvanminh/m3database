...
CREATE PROCEDURE [dbo].[p_MACC_DirectOfferTest]
...
BEGIN
	...
	--// \\10.3.0.40\Shared\MACC Offering
    BULK INSERT
        #TempMACCCustomer
    FROM
        '\\10.3.0.40\Shared\MACC Offering\MACC.csv'
    WITH
    (
        FIRSTROW = 2,
    ...
    SELECT
		...
		,[WebPortal].[Encrypt](CMR.CMR_ID) AS '[DOWTMA_Encrypted_CMR'
	...
	FROM 
		 [WebPortal].DOWTMA DTM
	INNER JOIN 
		iO_Client_MasterReference CMR ON CMR.CMR_ID = DTM.DOWTMA_CMR_ID
	LEFT JOIN 
		iO_Client_TypeIndividual CTI ON CTI.CTI_IDLink_CMR = DTM.DOWTMA_CMR_ID
	LEFT JOIN (
		SELECT 
	...
	LEFT JOIN (
		...
		FROM 
			[iO_Client_AddressDetail ]
		WHERE ISNULL(CAD_State, '') <> ''
		) 
			Address_State ON Address_State.CustomerPK = DTM.DOWTMA_CMR_ID AND Address_State.RowNumber = 1
		WHERE 
			DTM.DOWTMA_ID IN (SELECT DOWTMA_ID FROM @InsertOutput IOP)

        /*
        SELECT * FROM [WebPortal].[DOWTMA]


        EXEC [dbo].[p_MACC_DirectOfferTest] '{5ceb7f31-3b2c-4508-96ed-b8ac1c987650}', 'Money3 Campain'
