...
CREATE PROCEDURE [dbo].[spr_IdentifyRealRootClient]
@StartRow INT,
@EndRow INT
AS
BEGIN
	SELECT
		ROW_NUMBER() OVER (ORDER BY Root_Client_SeqNum ASC) AS Line,
		Root_Client_SeqNum
	INTO #t
	FROM (
		SELECT DISTINCT Root_Client_SeqNum
		FROM DuplicatorClientInfo
		--WHERE Root_Client_SeqNum = 510703
	) T
	...
	FROM #t
	WHERE
		Line >= @StartRow AND Line <= @EndRow

	DROP TABLE #t

	DECLARE @lineCount INT = (SELECT COUNT(*) FROM #tmpDCI)
	...
	WHILE @lineCount > 0
		BEGIN
			DECLARE @currentRoot VARCHAR(40) = (SELECT Root_Client_SeqNum FROM #tmpDCI WHERE Line = @lineCount)
			DECLARE @newRoot VARCHAR(40)
			...
			--SELECT * FROM #f
			--SELECT * FROM #finalResult ORDER BY Client_SeqNum

			SET @newRoot = (SELECT TOP 1 Client_SeqNum FROM #f)
			DECLARE @hasFunded BIT = 0
			IF (SELECT COUNT(1) FROM #f WHERE Funded > 0) >= 2
				SET @hasFunded = 1

			--PRINT @newRoot
			--PRINT 'HasFunded: ' + CONVERT(VARCHAR(10), @hasFunded)

			
			UPDATE DuplicatorClientInfo	SET 
				Root_Client_SeqNum = @newRoot,
				IsUpdated = 1,
				HasFunded = @hasFunded
			WHERE Root_Client_SeqNum = @currentRoot
			...
			--IF @currentRoot <> @newRoot
				--PRINT 'Old: ' + @currentRoot + ' - New: ' + @newRoot
