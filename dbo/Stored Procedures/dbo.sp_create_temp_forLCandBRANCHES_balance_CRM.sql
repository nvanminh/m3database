...
CREATE PROCEDURE [dbo].[sp_create_temp_forLCandBRANCHES_balance_CRM]
AS
BEGIN
	SET NOCOUNT ON;
	SET ANSI_WARNINGS OFF;

	DROP TABLE [reporting_loan].[dbo].[temptbl_LCandBRANCHES_westpac_CRM]
	...
	SELECT DISTINCT [Branch] = BR.CMR_Name
		...
		,[Client] = (
			SELECT REPLACE(REPLACE(cmr_name, '<cmr_name>', ''), '</cmr_name>', '')
		...
		,Contact.CTI_FirstName AS 'FirstName'
		,Contact.CTI_SurName AS 'SurName'
		,Contact.Mobile
		,Contact.Phone
		,Contact.email
		...
		,[Employer] = CASE 
			WHEN isnull(ced_idlink_xcet, '') = '{17ffdcef-dd9d-46ae-bdd4-4cab5502b517}'
				THEN (
						SELECT replace(replace(csep_tradingname, CHAR(10), ''), CHAR(13), '')
			...
			ELSE replace(replace(ced_employername_c, CHAR(10), ''), CHAR(13), '')
			END
		,[Position] = (
			SELECT replace(replace(XCO_Detail, CHAR(10), ''), CHAR(13), '') XCO_Detail
			FROM iO_Control_ClientOccupation WITH (NOLOCK)
			WHERE XCO_ID = CED_IDLink_Occupation_C
			)
		,[OccupationIndustry] = replace(replace(CED_IndustryType, CHAR(10), ''), CHAR(13), '') --CED_IndustryType
		...
		,[RegoExpiry] = vehicleSecurity.RegExpiry
		...
		,[AvailableRedraw] = 0
	INTO [reporting_loan].[dbo].[temptbl_LCandBRANCHES_westpac_CRM]
	...
	LEFT JOIN iO_Link_MasterReference primaryClient WITH (NOLOCK)
		ON primaryClient.LMR_IDLink_Code_ID = base.LMR_IDLink_Code_ID
			AND primaryClient.LMR_IDLink_Association IN ('{146afcaa-059b-469e-a000-0103e84144dc}') --Loan\Principal Borrower
	LEFT JOIN (
		SELECT CMR_ID
			,CTI_FirstName
			,CTI_surname
			...
			,CCD_Email.ccd_details as 'Email'
		...
		ON Contact.CMR_ID = primaryClient.LMR_IDLink_CMR
	...
	LEFT JOIN (
		...
		WHERE xat_detail = 'Residential'
			AND XAT_B2BKeyword LIKE '%Current'
		) AS CustAddress
	...
	LEFT JOIN (
		SELECT DISTINCT RSC_IDLink_RMR
			...
			,RSC_NewVehicle
			--,RSC_PPSR_Expirydate [RegExpiry]
			,RSC_RegoRenewal [RegExpiry]
