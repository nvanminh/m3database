...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[usp_Rpt_CT_PostFundingCheck]
	@StartDate date,
	@EndDate date
AS
BEGIN
...
--// Cash Train Loan
--//--------------------------------------------------------------------------------------------------------
IF OBJECT_ID('tempdb..#TempPostCashTrainLoan') IS NOT NULL
	DROP TABLE #TempPostCashTrainLoan;
...
--//--------------------------------------------------------------------------------------------------------
IF OBJECT_ID('tempdb..#TempPostCashTrainLoanToFund') IS NOT NULL
	DROP TABLE #TempPostCashTrainLoanToFund;
...
AND
	PT.RTM_IDLink_XTRM = '{71b22e14-385f-45a8-a389-36559632ff65}'  --// Direct Credit
AND
	PT.RTM_DateE BETWEEN @StartDate AND @EndDate
	--PT.RTM_DateE BETWEEN CAST((GETDATE() - 7) AS date) AND GETDATE()
)
...
--// Loan Date
--//--------------------------------------------------------------------------------------------------------
IF OBJECT_ID('tempdb..#TempPostLoanDate') IS NOT NULL
	DROP TABLE #TempPostLoanDate;
...
FROM
	(
		...
		FROM
			[dbo].[#TempPostCashTrainLoanToFund] TCTLTF
		LEFT OUTER JOIN
...
--// Loan Weekly Surplus
--//--------------------------------------------------------------------------------------------------------
IF OBJECT_ID('tempdb..#TempPostLoanWeeklySurplus') IS NOT NULL
	DROP TABLE #TempPostLoanWeeklySurplus;
...
--//--------------------------------------------------------------------------------------------------------
IF OBJECT_ID('tempdb..#TempPostLoanControlBalance') IS NOT NULL
	DROP TABLE #TempPostLoanControlBalance;
...
--// Loan Fees
--//--------------------------------------------------------------------------------------------------------
IF OBJECT_ID('tempdb..#TempPostLoanControlFee') IS NOT NULL
	DROP TABLE #TempPostLoanControlFee;
...
--// Client Occupation
--//--------------------------------------------------------------------------------------------------------
IF OBJECT_ID('tempdb..#TempPostClientOccupation') IS NOT NULL
	DROP TABLE #TempPostClientOccupation;
...
--//--------------------------------------------------------------------------------------------------------
IF OBJECT_ID('tempdb..#TempPostLoanEmployerSalary') IS NOT NULL
	DROP TABLE #TempPostLoanEmployerSalary;
...
--// Government Benefits
--//--------------------------------------------------------------------------------------------------------
IF OBJECT_ID('tempdb..#TempPostLoanGovernmentBenefits') IS NOT NULL
	DROP TABLE #TempPostLoanGovernmentBenefits;
...
SELECT
	...
	,GovernmentBenefits
INTO
	#TempPostLoanGovernmentBenefits
FROM
...
--//--------------------------------------------------------------------------------------------------------
IF OBJECT_ID('tempdb..#TempPostLoanPaymentFrequency') IS NOT NULL
	DROP TABLE #TempPostLoanPaymentFrequency;
...
--// Payment Schedule
--//--------------------------------------------------------------------------------------------------------
IF OBJECT_ID('tempdb..#TempPostPaymentScheduleDate') IS NOT NULL
	DROP TABLE #TempPostPaymentScheduleDate;
...
--// Repayment Amount
--//--------------------------------------------------------------------------------------------------------
IF OBJECT_ID('tempdb..#TempPostLoanRepaymentAmount') IS NOT NULL
	DROP TABLE #TempPostLoanRepaymentAmount;
...
--// Product C4
--//--------------------------------------------------------------------------------------------------------
IF OBJECT_ID('tempdb..#TempPostProductC4') IS NOT NULL
	DROP TABLE #TempPostProductC4;
...
--// Approval
--//--------------------------------------------------------------------------------------------------------
IF OBJECT_ID('tempdb..#TempPostApproval') IS NOT NULL
	DROP TABLE #TempPostApproval;
...
--//--------------------------------------------------------------------------------------------------------
IF OBJECT_ID(N'dbo.ZZ_Loan_PostFundingCheck', N'U') IS NULL
BEGIN
CREATE TABLE [dbo].[ZZ_Loan_PostFundingCheck](
	...
	[More_Than_2_Funded_SACC] [int] NULL,
	...
	[ExpenseSACCLoans] [decimal](18, 2) NULL
) ON [PRIMARY]
END

TRUNCATE TABLE [dbo].[ZZ_Loan_PostFundingCheck];
...
SELECT
	...
	,CASE WHEN tblFinalReport.CurrentLoanCount > 2 THEN 1 ELSE 0 END AS 'More_Than_2_Funded_SACC'
	...
	,tblFinalReport.ExpenseSACCLoans AS 'ExpenseSACCLoans'
FROM
(
SELECT
	RMR.RMR_ID
	,TCTLTF.LMR_Client_CMR
	,CAST(DATEDIFF(HOUR, CTI.CTI_DOB, TCTLTF.FundingEffectiveDate) / (365.23076923074 * 24) AS int) AS 'Age'
	...
	,(
		SELECT
			COUNT(*)
		FROM
			[dbo].[#TempPostCashTrainLoan] TCTL_CurrentLoan WITH (NOLOCK)
		INNER JOIN
	...
	,(
		SELECT
			COUNT(*)
		FROM
			[dbo].[#TempPostCashTrainLoan] TCTL_DefaultLoan WITH (NOLOCK)
		INNER JOIN
	...
	,(DATEDIFF(DAY, CAST(TCTLTF.FundingEffectiveDate AS date), TPSD.FirstPaymentDate) + 1) - (DATEDIFF(WEEK, CAST(TCTLTF.FundingEffectiveDate AS date), TPSD.FirstPaymentDate) * 2) - (CASE WHEN DATENAME(WEEKDAY, CAST(TCTLTF.FundingEffectiveDate AS date)) = 'Sunday' THEN 1 ELSE 0 END) - (CASE WHEN DATENAME(WEEKDAY, TPSD.FirstPaymentDate) = 'Saturday' THEN 1 ELSE 0 END) AS 'GracePeriod'
	--,TLTD.TermDay
	,DATEDIFF(DAY, TPSD.FirstPaymentDate, TPSD.LastPaymentDate) AS 'TermDay'
	,(((YEAR(TPSD.LastPaymentDate) - YEAR(CAST(TCTLTF.FundingEffectiveDate AS date))) * 12) + (MONTH(TPSD.LastPaymentDate) - MONTH(CAST(TCTLTF.FundingEffectiveDate AS date))) + (CASE WHEN DAY(TPSD.LastPaymentDate) > DAY(CAST(TCTLTF.FundingEffectiveDate AS date)) THEN 1 ELSE 0 END)) AS 'TermMonth'
	...
	,(
		SELECT TOP 1
			CASE
				WHEN PLLM.RLLm_IDLink_XFR = '{52A3A7A8-AF07-4327-B5AA-B66FB4288206}' THEN ISNULL(PLLM.RLLm_Value, 0.00) / (52.18 / 12) --// Monthly
				WHEN PLLM.RLLm_IDLink_XFR = '{529F0DF2-3394-4e32-B336-895699D75488}' THEN ISNULL(PLLM.RLLm_Value, 0.00) / 2.00         --// Fortnightly
				ELSE ISNULL(PLLM.RLLm_Value, 0.00)
			END AS 'SACCLoanWeekAmount'
		FROM
			[dbo].[iO_Product_LoanLiabilityMaster] PLLM
		WHERE
			PLLM.RLLm_IDLink_XLBo = '{17f1c7a7-241a-4430-b485-683174be4a19}' --// SACC Loans
		AND
			PLLM.RLLm_IDLink_RMR = RMR.RMR_ID
		ORDER BY
			PLLM.RLLm_SeqNumber
	) AS 'ExpenseSACCLoans'
FROM
	[#TempPostCashTrainLoanToFund] TCTLTF WITH (NOLOCK)
INNER JOIN
...
LEFT OUTER JOIN
	[dbo].[#TempPostLoanPaymentFrequency] TLPF WITH (NOLOCK) ON TLPF.RMR_ID = RMR.RMR_ID
LEFT OUTER JOIN
	[dbo].[#TempPostLoanRepaymentAmount] TLRA WITH (NOLOCK) ON TLRA.RMR_ID = RMR.RMR_ID
LEFT OUTER JOIN
	[dbo].[#TempPostLoanControlBalance] TLCB WITH (NOLOCK) ON TLCB.RMR_ID = RMR.RMR_ID
LEFT OUTER JOIN
	[dbo].[#TempPostLoanControlFee] TLCF WITH (NOLOCK) ON TLCF.RMR_ID = RMR.RMR_ID
LEFT OUTER JOIN
	[dbo].[#TempPostLoanDate] TLD WITH (NOLOCK) ON TLD.RMR_ID = RMR.RMR_ID
LEFT OUTER JOIN
	[dbo].[#TempPostClientOccupation] TCO WITH (NOLOCK) ON TCO.LMR_Client_CMR = TCTLTF.LMR_Client_CMR
LEFT OUTER JOIN
	[dbo].[#TempPostLoanEmployerSalary] TLES WITH (NOLOCK) ON TLES.RMR_ID = RMR.RMR_ID
LEFT OUTER JOIN
	[dbo].[#TempPostLoanGovernmentBenefits] TLGB WITH (NOLOCK) ON TLGB.RMR_ID = RMR.RMR_ID
LEFT OUTER JOIN
	[dbo].[#TempPostPaymentScheduleDate] TPSD WITH (NOLOCK) ON TPSD.RMR_ID = RMR.RMR_ID
LEFT OUTER JOIN
	[dbo].[#TempPostLoanWeeklySurplus] TLWS WITH (NOLOCK) ON TLWS.RMR_ID = RMR.RMR_ID
LEFT OUTER JOIN
	[dbo].[#TempPostProductC4] TPC4 WITH (NOLOCK) ON TPC4.RMR_ID = RMR.RMR_ID
LEFT OUTER JOIN
	[dbo].[#TempPostApproval] TA WITH (NOLOCK) ON TA.RMR_ID = RMR.RMR_ID
--WHERE
